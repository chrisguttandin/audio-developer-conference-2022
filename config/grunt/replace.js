...
module.exports = (grunt) => {
    return {
        'csp-production': {
            files: {
                'build/audio-developer-conference-2022/index.html': ['build/audio-developer-conference-2022/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: () => {
                            const html = fs.readFileSync('build/audio-developer-conference-2022/index.html', 'utf8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': ['build/audio-developer-conference-2022/ngsw.json']
            },
            options: {
                patterns: [
                    {
                    ...
                    },
                    {
                        ...
                        replacement: (_, filename) => {
                            return `"/audio-developer-conference-2022/${filename}.html": "${computeHashOfFile(
                                `build/audio-developer-conference-2022/${filename}.html`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
        ...
        'runtime': {
            files: {
                './': ['build/audio-developer-conference-2022/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, moduleAttribute) => {
                            if (moduleAttribute === undefined) {
                                return `<script>${fs.readFileSync(`build/audio-developer-conference-2022/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                            }

                            // eslint-disable-next-line node/no-sync
                            return `<script${moduleAttribute}>${fs.readFileSync(
                                `build/audio-developer-conference-2022/${filename}`
                            )}</script>`;
                        }
                    }
                ]
