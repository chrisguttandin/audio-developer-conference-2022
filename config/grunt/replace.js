...
module.exports = (grunt) => {
    return {
        'csp-production': {
            files: {
                'build/audio-developer-conference-2022/browser/index.html': ['build/audio-developer-conference-2022/browser/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: () => {
                            const html = fs.readFileSync('build/audio-developer-conference-2022/browser/index.html', 'utf8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': ['build/audio-developer-conference-2022/browser/ngsw.json']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, extension) =>
                            grunt.file.expand({ cwd: 'build/audio-developer-conference-2022', ext: extension }, `assets/${filename}.*`)[0]
                    },
                    {
                        match: /\s*"\/audio-developer-conference-2022(?:\/scripts)?\/runtime(?:-es(?:2015|5))?.[\da-z]*\.js",/g,
                        replacement: ''
                    },
                    {
                        match: /\s*"\/audio-developer-conference-2022(?:\/scripts)?\/runtime(?:-es(?:2015|5))?.[\da-z]*\.js":\s*"[\da-z]+",/g,
                        replacement: ''
                    },
                    {
                        // Replace the hash value inside of the hashTable for "/(index|start).html" because it was modified before.
                        match: /"\/audio-developer-conference-2022\/(?<filename>index|start)\.html":\s*"[\da-z]+"/g,
                        replacement: (_, filename) => {
                            return `"/audio-developer-conference-2022/browser/${filename}.html": "${computeHashOfFile(
                                `build/audio-developer-conference-2022/browser/${filename}.html`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
        ...
        'runtime': {
            files: {
                './': ['build/audio-developer-conference-2022/browser/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, moduleAttribute) => {
                            if (moduleAttribute === undefined) {
                                return `<script>${fs.readFileSync(`build/audio-developer-conference-2022/browser/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                            }

                            // eslint-disable-next-line node/no-sync
                            return `<script${moduleAttribute}>${fs.readFileSync(
                                `build/audio-developer-conference-2022/browser/${filename}`
                            )}</script>`;
                        }
                    }
                ]
