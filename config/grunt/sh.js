    ...
    return {
        ...
        'lint-src': {
            cmd: `htmlhint --config config/htmlhint/document.json 'src/**/index.html' && \
                ...
                npx ng lint audio-developer-conference-2022 && \
                npx stylelint 'src/**/*.scss' --config config/stylelint/config.json`
        },
        'lint-test': {
            cmd: 'npx ng lint audio-developer-conference-2022 --configuration test'
        },
        'monitor': {
            cmd: 'npx ng serve'
        },
        'prerender': {
            cmd: `npx ng run audio-developer-conference-2022:server:production && \
                npx angular-prerender \
                    --browser-target audio-developer-conference-2022:build \
                    --preserve-index-html \
                    --server-target audio-developer-conference-2022:server`
        },
        'preview': {
        ...
        'verify': {
            cmd: `npx bundle-buddy build/audio-developer-conference-2022/*.js.map && \
